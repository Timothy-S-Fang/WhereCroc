...
hMM = function( bp1, bp2, prevStateProbs, edges, observations, transMatrix) {
  stateProbabilities = matrix(0, 40, 40)
  
  for (i in 1:40) {
    if (i != bp1 && i != bp2) {
      options = getOptions(i, edges)
      for (opt in options) {
        stateProbabilities[i, opt] = prevStateProbs[i] * transMatrix[i, opt]
      }
    }
  }
  ...
  for (i in 1:40) {
    if (i != bp1 && i != bp2) {
      transProb[i] = sum(stateProbabilities[, i]) * observations[i]
    }
  }
  
  transProb = transProb / sum(transProb)  
  
  return(transProb)
}

initialization = function(bp1, bp2, rangerPos) {
  s0 = rep(1 / 37, 40)
...
getTransMatrix = function(edges) {
  transMatrix = matrix(0, 40, 40)
  
  for (i in 1:40) {
    neighbours = getOptions(i, edges)
    prob = 1 / length(neighbours)
    for (n in neighbours) {
      transMatrix[i, n] = prob
    }
  }
...
getTransValue = function(currentNode, nextNode, edges) {
  neighbours = getOptions(currentNode, edges)
  
  if (nextNode %in% neighbours) {
...
calculateEmissionMatrix = function(probs, readings) {
...
calculateShortestPath = function(source, destination, edges) {
  visited = c()
  frontier = c()
  ...
  while (expandedNode != destination) {
    ...
    for (node in neighbours) {
      if (!node %in% frontier && !node %in% visited) {
...
myFunction = function(moveInfo, readings, positions, edges, probs) {
  e = calculateEmissionMatrix(probs, readings)
  stateProbabilities = rep(0, 40)
  currentNode = positions[3]
  bp1 = ifelse(is.na(positions[1]), 0, positions[1])
  bp2 = ifelse(is.na(positions[2]), 0, positions[2])
  
  if (moveInfo$mem$status < 2) {
    # set Intitial
    s0 = initialization(bp1, bp2, currentNode)
    moveInfo$mem$status = 2
    moveInfo$mem$s0 = s0
    
    stateProbabilities = s0 * e
    stateProbabilities = stateProbabilities / sum(stateProbabilities)
    cl = which(stateProbabilities == max(stateProbabilities))
    
  } else if (bp1 < 0 || bp2 < 0) {
    # check backpackers
    if (bp1 < 0) {
      cl = -1 * bp1
      stateProbabilities[cl] = 1
    }
    if (bp2 < 0) {
      cl = -1 * bp2
      stateProbabilities[cl] = 1
    ...
    s0 = moveInfo$mem$s0
    transMatrix = getTransMatrix(edges)
    stateProbabilities = hMM(bp1, bp2, s0, edges, e, transMatrix)
    cl = which(stateProbabilities == max(stateProbabilities))
  }
  
  path = calculateShortestPath(currentNode, cl, edges)
  ...
  if (length(path) >= 2) {
    ...
    stateProbabilities[cl] = 0
  } else if (length(path) == 0) {
    moveInfo$moves = c(0, 0)
    stateProbabilities[cl] = 0
  }
  
  moveInfo$mem$s0 = stateProbabilities
